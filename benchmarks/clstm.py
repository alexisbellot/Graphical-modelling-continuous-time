    ...
    def forward(self, X, hidden=None, truncation=None):
        ...
        if truncation is None:
            ...
            for t in range(0, T, truncation):
                lstm_out, hidden = self.lstm(X[:, t : min(t + truncation, T)], hidden)
...
class cLSTM(nn.Module):
    ...
    def forward(self, X, i=None, hidden=None, truncation=None):
        ...
        if i is None:
            ...
            pred = [self.networks[i](X, hidden[i], truncation) for i in range(self.p)]
...
class cLSTMSparse(nn.Module):
    ...
    def forward(self, X, i=None, hidden=None, truncation=None):
        ...
        if i is None:
            if hidden is None:
                hidden = [None for _ in range(self.p)]
            pred = [self.networks[i](X[:, :, self.sparsity[i]], hidden[i], truncation) for i in range(self.p)]
            pred, hidden = zip(*pred)
            pred = torch.cat(pred, dim=2)
        else:
...
def prox_update(network, lam, lr):
    ...
    W.data = (W / torch.clamp(norm, min=(lam * lr * 0.1))) * torch.clamp(norm - (lr * lam), min=0.0)
    ...
    for it in range(max_iter):
        ...
        if (it + 1) % check_every == 0:
            ...
            if verbose > 0:
                ...
                print("Variable usage = %.2f%%" % (100 * torch.mean(clstm.GC().float())))
...
def train_model_adam(clstm, X, lr, niter, check_every, truncation=None, verbose=0):
...
def NGC_LSTM(data):
    ...
    p = data.shape[1]
    clstm = cLSTM(p, hidden=5)
    check_every = 100
    train_loss_list, train_mse_list = train_model_gista(
        clstm, X, lam=6.6, lam_ridge=1e-4, lr=0.005, max_iter=2000, check_every=check_every, truncation=5
    )
